...
def obtener_mueble(ref, modulos):
...
def procesar_especial(nombre, medidas, colocacion_usada, tipo_mueble, especiales_dict):
    salida = []
    incremento_total = 0
...
def analizar_mueble(ref, ancho, alto, profundo, especiales_input, modulos, especiales_dict):
    mueble = obtener_mueble(ref, modulos)
    if mueble is None:
    ...
    for esp in especiales_input:
        ...
        resultados, inc = procesar_especial(nombre, medidas_disponibles, colocacion_usada, tipo_mueble, especiales_dict)
